...
- Information collection currently targets Bilibili video workflows. Electron main owns platform requests and Chrome cookie reuse, while `electron/informationCollectionRuntime.ts` owns tool detection, external command execution, run ids, subtitle text reading, and tool-failure wording. Each processing run writes to `runtime/information-collection/bilibili/{bvid}/{runId}` and reports `information-collection:process-progress` to the requesting renderer only. Formal output is the existing node Word document snapshot; runtime cookies, subtitles, audio, article HTML, and transcription intermediates must not be packaged.
