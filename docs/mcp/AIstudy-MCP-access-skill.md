...
- `write_node_document` 和 `append_node_document` 的 `text` 必须保持干净并结构化：一级标题、短步骤标题、`目标：`/`数据来源：`/`推荐 Action:` 字段标签、编号或项目列表、简洁正文；独立知识点之间保留且只保留一个空行，不要用额外空行制造视觉间距；不要把 Mermaid 或 Markdown fenced block 原样写入正文，真实导图结构优先使用导图工具，文档内需要说明时改写成标题和列表。
