...
远程文档写入同样遵循本地 MCP 规范：使用结构化文本，拆成标题、短步骤标题、字段标签、编号/项目列表和简洁正文；独立知识点之间空一行，不用额外空行制造视觉间距；不要把 Mermaid 或 Markdown fenced block 原样写入正文，真实导图结构优先使用导图工具，文档内需要说明时改写成标题和列表；数学内容使用 `ε`、`δ`、`∞`、`→`、`≤`、`≥`、`x_n`、`x^2`、`lim_{n→∞}` 等规范表达，不把 `epsilon`、`infinity`、`->` 等退化文本写入最终文档。
