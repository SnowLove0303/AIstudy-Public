...
远程读取同样执行严格范围规则：`read_current_mindmap`、`search_nodes`、`list_node_documents` 缺少 `courseId` 时不会默认全库读取；调用方必须显式传 `scope: "all"`。节点文档默认使用轻量 `text` 模式，需要编辑器 JSON 时才请求 `snapshot` 模式。长正文必须检查 `complete` 并按 `nextOffset` 续读；`read_node_context` 必须检查顶层 `completion`，需要一次取得目标范围全文时才使用 `documentMode: "full"`。
...
远程文档写入同样遵循本地 MCP 中国文章排版规范：节点名称已作为文档抬头，正文默认不重复；独立文章标题使用 `#`，正文使用 `一、`/`（一）`/`1.`/`（1）` 四级标题、引用、字段标签、列表和逐段正文。新正文自动应用宋体、两字符首行缩进、两端对齐和比例行距；输入空行只标记段落边界。中文标点与中英文间距会安全规范化，同时逐字符保护 Windows/UNC 路径、紧凑引用、JSON、命令开关、工具名、字段名、ID、脚本名、URL、邮箱、代码、公式、列表符号和树形缩进。真实导图仍优先使用导图工具。

远程文档写入已有内容时必须传最近一次读取到的 `currentSnapshotId` 作为 `expectedSnapshotId`。版本冲突会拒绝写入，避免覆盖本机 UI 或其他客户端刚保存的内容。

文档标题为空或为通用“节点文档”时，MCP 有效标题回退到节点名称。写入成功只证明 MySQL 文档快照已提交；当前没有可验证向量索引时，返回 `ragIndex.status: "not_configured"` 和 `synchronized: false`，远程调用方不得把它解释为 RAG 已同步。
...
- 远程编辑权限按组放行，删除操作独立控制。
- 本机 stdio 的环境级工具/知识库/节点白名单是独立的第二层策略；远程入口仍以设置页权限组、token、Origin 和远程工具白名单为准。
