...
远程文档写入同样遵循本地 MCP 中国文章排版规范：节点名称已作为文档抬头，正文默认不重复；独立文章标题使用 `#`，正文使用 `一、`/`（一）`/`1.`/`（1）` 四级标题、引用、字段标签、列表和逐段正文。新正文自动应用宋体、两字符首行缩进、两端对齐和比例行距；输入空行只标记段落边界。中文标点与中英文间距会安全规范化，同时保护 URL、Windows 路径、邮箱、代码、公式、列表符号和树形缩进。真实导图仍优先使用导图工具。
