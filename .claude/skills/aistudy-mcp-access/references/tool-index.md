...
- `append_node_document`: append clean text or Markdown-style headings. Use field labels and lists for structured content. Keep exactly one blank line between independent knowledge points and avoid degraded math tokens such as `epsilon`, `infinity`, `->`, or `lim_{n->infinity}`.
