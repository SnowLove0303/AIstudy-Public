...
function getAistudyDataRoot() {
  const configuredDataRoot = process.env.AISTUDY_PUBLIC_DATA_ROOT?.trim();
...
function getChromePortRuntimeRoot() {
  const configuredRoot = process.env.AISTUDY_PUBLIC_RUNTIME_ROOT?.trim();
...
async function readMysqlConfig(): Promise<MysqlConfig> {
  ...
  const config = {
    host: getStringSetting(process.env.AISTUDY_PUBLIC_MYSQL_HOST, getStringSetting(readSetting(mergedConfig, "host"), "127.0.0.1")),
    port: parsePort(process.env.AISTUDY_PUBLIC_MYSQL_PORT, parsePort(readSetting(mergedConfig, "port"), 3306)),
    user: getStringSetting(process.env.AISTUDY_PUBLIC_MYSQL_USER, getStringSetting(readSetting(mergedConfig, "user"), "root")),
    password: typeof process.env.AISTUDY_PUBLIC_MYSQL_PASSWORD === "string"
      ? process.env.AISTUDY_PUBLIC_MYSQL_PASSWORD
      : getStringSetting(readSetting(mergedConfig, "password"), ""),
    database: getStringSetting(process.env.AISTUDY_PUBLIC_MYSQL_DATABASE, getStringSetting(readSetting(mergedConfig, "database"), "aistudy_public")),
    courseTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_COURSE_TABLE,
      getStringSetting(readSetting(mergedConfig, "courseTable"), "course_management_courses")
    ),
    courseSectionTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_COURSE_SECTION_TABLE,
      getStringSetting(readSetting(mergedConfig, "courseSectionTable"), "knowledge_sections")
    ),
    mindMapTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_MIND_MAP_TABLE,
      getStringSetting(readSetting(mergedConfig, "mindMapTable"), "mind_maps")
    ),
    mindMapSnapshotTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_MIND_MAP_SNAPSHOT_TABLE,
      getStringSetting(readSetting(mergedConfig, "mindMapSnapshotTable"), "mind_map_snapshots")
    ),
    mindMapNodeTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_MIND_MAP_NODE_TABLE,
      getStringSetting(readSetting(mergedConfig, "mindMapNodeTable"), "mind_map_nodes")
    ),
    knowledgeDocumentTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_KNOWLEDGE_DOCUMENT_TABLE,
    ...
    knowledgeDocumentSnapshotTable: getStringSetting(
      process.env.AISTUDY_PUBLIC_MYSQL_KNOWLEDGE_DOCUMENT_SNAPSHOT_TABLE,
...
async function downloadUpdate(downloadUrlValue: unknown): Promise<UpdateDownloadResult> {
  ...
  const fileName = decodeURIComponent(path.basename(url.pathname)) || `AIstudy Public-Setup-${app.getVersion()}.exe`;
...
async function getUpdateManagerInfo(): Promise<UpdateManagerInfo> {
  ...
  const installerPath = path.join(releaseDir, `AIstudy Public-Setup-${app.getVersion()}.exe`);
