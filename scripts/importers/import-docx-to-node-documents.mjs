...
async function loadMysqlConfig() {
  ...
  return {
    host: process.env.AISTUDY_PUBLIC_MYSQL_HOST || fileConfig.host || "127.0.0.1",
    port: Number(process.env.AISTUDY_PUBLIC_MYSQL_PORT || fileConfig.port || 3306),
    user: process.env.AISTUDY_PUBLIC_MYSQL_USER || fileConfig.user || "root",
    password: process.env.AISTUDY_PUBLIC_MYSQL_PASSWORD ?? fileConfig.password ?? "",
    database: process.env.AISTUDY_PUBLIC_MYSQL_DATABASE || fileConfig.database || "aistudy_public"
  };
}
