...
- The selected mind-map node title is the single source of truth for the document heading. The renderer shows it above the editable body and DOCX export prepends it as the visible heading; it is not duplicated into the editor snapshot, so node renames cannot leave stale document titles or trigger snapshot rewrites.
